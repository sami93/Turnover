...
export class DataSetService {
  ...
  getDataSetbyMatricule(dataset): Observable<any> {
    return this.http.get(`/api/datasetsMat/${dataset.Matricule}`).map(res => res.json());
  }

  editDataSet(dataset): Observable<any> {
    return this.http.put(`/api/dataset/${dataset._id}`, JSON.stringify(dataset), this.options);
  }

  deleteDataSet(dataset): Observable<any> {
    return this.http.delete(`/api/dataset/${dataset.Matricule}`, this.options);
  }
